...
class PlasmaConfig:
    ...
    def __str__(self):
        niter = int(2000000 + int(8e7/(self.He_pressure/1e-6)))
        string =  """
...
"""
        string += "t_equilib = " + str(int(niter*0.75)-500000) + """
...
pressures = [1e-3, 3e-4, 1e-4, 3e-5, 1e-5, 3e-6, 1e-6, 3e-7, 1e-7]
#import os
i=-1
config = PlasmaConfig()
for p in pressures :
    ...
    print >>f, config
    f.close()
    command = "echo 'y'|./plasma2d config=config_%d.txt output_dir=%.0eTorr" % (i,p)
    #if (i+1) % 2 != 0 :
	#command += " &"	
    print command
    #os.system(command)
